...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    public function testFlushOrdering()
    {
        ...
        // 4) Add another route
        $routes->add('/admin', 'AppBundle:Admin:dash', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/admin', 'AppBundle:Admin:dash', 'admin_dashboard');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        $adminRoutes->add('/dash', 'AdminController::dashboardAction', 'admin_dashboard');
        ...
        $collection = $routes->build();
        $this->assertEquals('/admin/dash', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
    ...
    public function testAutomaticRouteNamesDoNotConflict()
    {
        ...
        $adminRoutes = $routes->createBuilder();
        // route 1
        $adminRoutes->add('/dash', '');

        $accountRoutes = $routes->createBuilder();
        // route 2
        $accountRoutes->add('/dash', '')
            ->setMethods(['GET']);
        // route 3
        $accountRoutes->add('/dash', '')
            ->setMethods(['POST']);
